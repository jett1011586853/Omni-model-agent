...
- Units can now be queued with stored activation requests instead of being started immediately.
- When a dependency completes, waiting auto-activated units can be promoted and run without the caller re-issuing the unit.
- Targets can stay `waiting` until prerequisite targets complete, then promote their member units into activation.
- Unit `OnSuccess` and `OnFailure` hooks can enqueue follow-up units.

## Next high-value steps

- Teach the scheduler to read target criticality and DAG depth, not only unit-local health signals.
- Add richer target semantics such as `Wants` vs `Requires`, `Before`/`After` at the target level, and `OnFailure` recovery targets.
- Let retriever/compressor consume more of the mirrored VFS tree directly, not only SQLite-backed memories.
- Add an immune-system layer for loop detection, tool abuse detection, and automatic circuit breakers.
